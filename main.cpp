...
// Camera parameters.
///////////////////////////////////////////////////////////////////////////////
vec3 cameraPosition(0.0f, 100.0f, 150.0f);
vec3 cameraDirection = normalize(vec3(0.0f, -50.0f, -100.0f) - cameraPosition);
...
void initialize()
{
	...
	// Initialize terrain
	///////////////////////////////////////////////////////////////////////
	// Start with LOW tesselation for testing!
	terrain.generateMesh(512);  
...
void display(void)
{
	...
	// ============================================
	// 2. THEN draw other objects (spaceship, landing pad)
	// ============================================
	drawScene(shaderProgram, viewMatrix, projMatrix, lightViewMatrix, lightProjMatrix);
	debugDrawLight(viewMatrix, projMatrix, vec3(lightPosition));

	// ============================================
	// 3. FINALLY draw terrain
	// ============================================
	//use the terrain shader program for rendering
	glUseProgram(heightfieldShaderProgram);

	
	// --------------------------------------------
	// Bind heightfield texture (displacement)
	// --------------------------------------------
	glActiveTexture(GL_TEXTURE0);
	glBindTexture(GL_TEXTURE_2D, terrain.m_texid_hf);
	// The highfield shader expects the height texture at binding = 0
	glUniform1i( 
		glGetUniformLocation(heightfieldShaderProgram, "heightTexture"), 0
	);

	// --------------------------------------------
	// Bind aerial photo (diffuse texture)
	// IMPORTANT: shading.frag expects this at binding = 0
	// --------------------------------------------
	glActiveTexture(GL_TEXTURE1);
	glBindTexture(GL_TEXTURE_2D, terrain.m_texid_diffuse);
	// Tell shader that we have a color texture
	glUniform1i(
		glGetUniformLocation(heightfieldShaderProgram,  "colorMap" ), 1
	);

	// Tell the fragment shader to sample from texture unit 1 for colors
	glUniform1i(glGetUniformLocation(heightfieldShaderProgram, "has_color_texture"), 1);

	// --------------------------------------------
	// Bind environment maps
	// --------------------------------------------
	glUniform1i(glGetUniformLocation(heightfieldShaderProgram, "environmentMap"), 6);
	glUniform1i(glGetUniformLocation(heightfieldShaderProgram, "irradianceMap"), 7);
	glUniform1i(glGetUniformLocation(heightfieldShaderProgram, "reflectionMap"), 8);


	// --------------------------------------------
	// Heightfield parameters
	// --------------------------------------------
	// How tall the terrain is (scaling factor for displacement)
	glUniform1f(glGetUniformLocation(heightfieldShaderProgram, "heightScale"), 50.0f);
	...
	glUniform3fv(
		glGetUniformLocation(heightfieldShaderProgram, "viewSpaceLightPosition"),1,&viewSpaceLightPosition[0]);

	glUniform3fv(
		glGetUniformLocation(heightfieldShaderProgram, "point_light_color"),1,&point_light_color[0]);

	glUniform1f(
		glGetUniformLocation(heightfieldShaderProgram, "point_light_intensity_multiplier"),
		point_light_intensity_multiplier
	);
	...
	glUniform1f(
		glGetUniformLocation(heightfieldShaderProgram, "material_fresnel"),
		0.04f
	);
	glUniform1f(
	...
	glUniformMatrix4fv(
		...
		1,
		GL_FALSE,
		&terrainModelMatrix[0][0]
	);

	glUniformMatrix4fv(
		glGetUniformLocation(heightfieldShaderProgram, "viewMatrix"),
		1,
		GL_FALSE,
		&viewMatrix[0][0]
	);
	...
	glUniformMatrix4fv(
		glGetUniformLocation(heightfieldShaderProgram, "projectionMatrix"),
		1,
		GL_FALSE,
		&projMatrix[0][0]
	);
	...
	glUniformMatrix4fv(
		glGetUniformLocation(heightfieldShaderProgram, "viewInverse"),
		1,
		GL_FALSE,
		&viewInverse[0][0]
	);
